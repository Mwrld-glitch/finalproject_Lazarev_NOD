    ...
    def run_update(self) -> bool:
        ...
        try:
            ...
            if crypto_rates:
                ...
                print(f"INFO: Fetching from CoinGecko... OK ({len(crypto_rates)} rates)")
                success_sources += 1
            else:
        ...
        try:
            ...
            if fiat_rates:
                ...
                print(f"INFO: Fetching from ExchangeRate-API... OK ({len(fiat_rates)} rates)")
                success_sources += 1
            else:
        ...
        for currency_pair, rate in all_rates.items():
            current_rates_data[currency_pair] = {
                ...
                "source": "CoinGecko" if any(crypto in currency_pair for crypto in ["BTC", "ETH", "SOL"]) else "ExchangeRate-API"
            }

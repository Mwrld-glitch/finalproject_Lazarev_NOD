    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            try:
                ...
                # Формируем лог
                log_msg = f"{action} user='{user_id}' currency='{currency}' amount={amount}"
                ...
                result = "ERROR"
                user_id = args[0] if len(args) > 0 else "unknown"
                logger.error(f"{action} user='{user_id}' result={result} error_type={type(e).__name__} error_message='{str(e)}'")
                raise
